...
#🔥 «О» — ОПОЗДАЛ (3.7.6), а не «уважительная причина». Метку переназначили по факту
#того, как её понимали преподаватели; отдельной метки для уважительной причины больше
#нет (решение Ярослава 19.08.2026).
#⚠️ Ключ действия остался прежним — `absent_o`: он живёт в контракте с веб-диалогом
#подтверждения и в сотне тестов, и переименование ради красоты стоило бы дороже пользы.
#Читать его надо как «метка О», а не как «absent».
#⚠️ Прежние формулировки («отпросился», «на соревнованиях», «по справке») ОТСЮДА УБРАНЫ
#намеренно: оставить их значило бы записывать отпросившегося как опоздавшего. Теперь
#такая фраза не даёт метки вовсе, и Вектор честно переспрашивает.
_ABSENCE_O = ("опозда", "припозд", "задержал", "пришел позже", "пришёл позже",
              "пришла позже", "не успел к начал", "не успела к начал",
              "с опоздани", "позже начала")
...
#⚠️ Кеш на чистых функциях строки. Разбор одной голосовой команды по ростеру в 30
#человек звал `_stem` СОТНИ раз, причём на одних и тех же словах: и по каждому слову
#запроса для каждой фамилии, и повторно при доразборе однофамильцев. Функции
#детерминированные и без побочных эффектов, поэтому кеш здесь не «ускорение на глазок»,
#а устранение заведомо повторной работы.
#Размер ограничен: ростер колледжа это тысячи фамилий, безлимитный кеш рос бы вместе с
#числом разобранных команд и никогда не отдавал бы память.
@lru_cache(maxsize=4096)
def _stem(word: str) -> str:
    s = _norm(word).strip()
    #⚠️ Цикл, а не `endswith(кортеж)`: нам нужно не «оканчивается ли», а КАКОЕ именно
    #окончание отрезать, и с проверкой длины у каждого. `endswith` по кортежу ответил бы
    #только «да/нет», после чего пришлось бы всё равно искать совпавшее окончание —
    #то есть пройти список второй раз.
    for end in _ENDINGS:
...
def _absence_kind(text: str) -> str:
    """Метка посещаемости по ключевым основам или '' если она не упомянута. Приоритет:
    опоздание > болезнь > неявка. Опоздание проверяем ПЕРВЫМ: «опоздал на пару» содержит
    и слово «пара», и по смыслу это НЕ пропуск — человек на занятии был."""
    t = _norm(text)
...
def _match_students(text: str, roster: list[tuple[str, str]],
    ...
    #🔥 Основы слов запроса считаются ОДИН раз, а не заново для каждой фамилии. Раньше
    #`_stem(w)` стоял ВНУТРИ цикла по ростеру: при 30 студентах и 5 словах это 150
    #одинаковых вычислений на команду, и все — с созданием новых строк в памяти.
    #Результат тот же до последнего знака: `_stem` — чистая функция.
    ...
    for (f, n) in roster:
        ...
        best = 0.0
        for w in stems:
            # ⚠️ ОТСЕЧЕНИЕ СЧИТАЕТСЯ ПО ОБЕИМ ФОРМАМ, И ЭТО КУПЛЕНО ОШИБКОЙ.
            # У `SequenceMatcher.ratio()` есть жёсткая верхняя граница
            # 2·min(|a|,|b|)/(|a|+|b|): совпасть может в лучшем случае вся короткая
            # строка целиком. Не превышает найденного лучшего — считать точное значение
            # незачем, максимум оно не изменит.
            #
            # 🔥 НО `_similar` берёт МАКСИМУМ из двух сравнений: прямого и фонетического,
            # а `_phon` строку УКОРАЧИВАЕТ (удвоенные буквы схлопываются, ь и ъ
            # удаляются). Граница, посчитанная по исходным длинам, для фонетической
            # формы занижена: у «хаанн»/«хан» она даёт 0.75, а фонетические формы
            # совпадают ПОЛНОСТЬЮ — 1.0. Пара отбрасывалась, и настоящий студент
            # проигрывал соседу по ростеру. Ровно тот случай, ради которого фонетика и
            # заведена: Whisper слышит бурятские ФИО с удвоениями.
            #
            # Поэтому берём максимум двух границ — по исходным формам и по фонетическим.
            # Обе величины уже посчитаны и закешированы, лишней работы нет.
            lw = len(w)
            pw = _phon(w)
            lpw = len(pw)
            bound = 0.0
            if lw and lf:
                bound = 2.0 * min(lw, lf) / (lw + lf)
            if lpw and lpf:
                bound = max(bound, 2.0 * min(lpw, lpf) / (lpw + lpf))
            if bound <= best:
                continue
            r = _similar(w, fst)
            if r > best:
...
def parse(text: str, roster: list[tuple[str, str]],
    ...
    if absence:
        ...
        base.error = ("Не понял действие. Скажите оценку (2–5) или "
                      "«пропуск» / «болеет» / «опоздал» / «был на паре».")
        return base
...
def stt_context(roster: list[tuple[str, str]]) -> str:
    ...
    return ("Журнал успеваемости колледжа. Оценки: два, три, четыре, пять, "
            ...
            "опоздал, задержался, был на паре, присутствовал. "
...
_LESSON_CREATE = ("созда", "добавь занят", "добавь лекц", "добавь практ", "добавь семинар",
                  "добавь дз", "добавь домашн", "новое занят", "новую лекц", "новую практ",
                  "проведи", "запиши занят", "поставь занят", "заведи занят",
                  "запланируй занят", "задай дз", "задай домашн")
#Порядок ВАЖЕН: основы проверяются сверху вниз и первая совпавшая побеждает. «Домашнее
#задание» стоит выше «зачет»/«практик» намеренно — фраза «задай домашнее задание по
#практике» должна создать ДЗ, а не практику.
_LESSON_TYPES = [("домашн", "ДЗ"), ("дз", "ДЗ"),
                 ("лаборатор", "Лабораторная"), ("практик", "Практика"),
                 ("лекци", "Лекция"), ("семинар", "Семинар"), ("экзамен", "Экзамен"),
                 ("зачет", "Зачёт")]


def _type_stem_hit(stem: str, low: str) -> bool:
    """Совпала ли основа типа занятия. Короткие основы («дз») ищем как ОТДЕЛЬНОЕ СЛОВО:
    подстрокой они всплывали бы внутри обычных слов («надзор», «подзаголовок») и молча
    подменяли тип занятия. Длинные основы остаются подстрокой — так они ловят склонения."""
    if len(stem) <= 3:
        return re.search(rf"\b{re.escape(stem)}\b", low) is not None
    return stem in low
...
def parse_batch(text: str, roster: list[tuple[str, str]],
    ...
    if (any(t in low for t in _LESSON_CREATE)
            and any(_type_stem_hit(s, low) for s, _ in _LESSON_TYPES)):
