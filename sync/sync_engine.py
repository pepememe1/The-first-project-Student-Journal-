...
def _collect_lessons() -> list:
    ...
    for r in rows:
        out.append({
            ...
            #⚠️ `extra` НЕ ШЛЁМ ВОВСЕ, и это починка потери данных (20.09.2026,
            #находка ревью J03). Здесь стояло `"extra": {}` — молчаливая неправда:
            #у локальной таблицы `lessons` такой колонки нет, знать содержимое поля
            #десктоп не может, а пустой словарь сервер читал как «человек очистил» и
            #затирал им даты пересдач 2–5, заполненные в вебе. Отсутствующий ключ
            #сервер не применяет вовсе — ровно так же, как `subgroup`, которого тут
            #нет по той же причине. Молчание честнее пустоты.
            "hour": r[8], "updated_at": r[9] or _now(),
            "deleted": bool(r[10]),
...
#docs/research/SYNC-RESEARCH-2026.md.
#
...
def reconcile(client) -> bool:
    ...
    with _session.lock:
        ...
        try:
            ...
            _session.collect_failed = False   #флаг — про ЭТОТ сбор, а не про прошлый цикл
            res = _push_all(client, collect_local()) or {}
        except Exception as e:
            _log.warning("пуш офлайн-правок перед сбросом кэша не удался: %s", e)
            raise

        #🔥 «PUSH НЕ УПАЛ» ЕЩЁ НЕ ЗНАЧИТ «ПРАВКИ СОХРАНЕНЫ» (20.09.2026, находка ревью
        #J04). Здесь проверялось только исключение — а у отправки есть ДВА тихих исхода,
        #при которых она возвращает успех, и оба заканчивались стиранием кэша:
        #
        #1. СОБРАЛИ НЕ ВСЁ. Коллектор при сбое чтения таблицы возвращает пустой список и
        #   ставит `collect_failed` (ронять весь цикл из-за одной залоченной таблицы
        #   нельзя). Пустота неотличима от «изменений нет»: push проходит, сервер этих
        #   строк не видел НИКОГДА — и следом они стираются с единственной машины, где
        #   лежали. `sync_once` этот флаг читает (метку дельты не двигает), а reconcile
        #   не читал, хотя цена ошибки здесь несравнимо выше: там отсрочка, тут потеря.
        #2. СЕРВЕР ЧАСТЬ ОТВЕРГ. `rejected` — штатный и живой случай: админ снял
        #   назначение преподавателя на предмет, пока тот работал офлайн. Ответ при этом
        #   успешный, и отвергнутые оценки существовали ровно в одном месте — в кэше,
        #   который стирается следующей строкой.
        if _session.collect_failed:
            #Сбой временный (таблица разблокируется) — повтор следующим циклом осмыслен,
            #поэтому исключение: вызывающий не снимет флаг сверки.
            raise RuntimeError("часть локальных таблиц не прочиталась — сверку "
                               "откладываю, кэш не трогаю")
        _session.last_rejected = dict(res.get("rejected") or {})
        if _session.last_rejected:
            #А это состояние СТОЙКОЕ: пока админ не вернёт назначение, повтор даст тот же
            #отказ. Бросать исключение нельзя — сверка пошла бы по кругу каждые 30 с, и
            #каждый круг это полный снимок базы, самый дорогой обмен из всех. Поэтому:
            #кэш НЕ стираем, обмен выполняем обычным циклом и считаем сверку
            #состоявшейся настолько, насколько это безопасно. Цена названа честно —
            #«осиротевшие» локальные записи в этот раз не убраны.
            _log.error("сервер отклонил часть правок (%s) — сверку «сервер = истина» "
                       "делаю БЕЗ очистки кэша: отклонённое живёт только здесь",
                       _session.last_rejected)
            return _sync_once_locked(client)

        #⚠️ Снимок ПЕРЕД стиранием — это доступный нам уровень атомарности. Настоящая
        #замена «скачали новое, потом подменили» потребовала бы тянуть полный pull во
        #временное хранилище, а merge пишет прямо в базу. Зато откат есть: следующий pull
        #оборвался — возвращаем файл, и человек остаётся с данными, а не с пустотой.
        snapshot = DBManager.backup(reason="before_reconcile")
        if not snapshot and _os.path.exists(LOCAL_DB):
            #База есть, а копия не снялась (чаще всего — кончилось место). Стирать без
            #возможности вернуть нельзя: это ровно тот случай, ради которого копия и
            #делается. Обмен выполняем, очистку пропускаем.
            _log.error("копия базы перед сверкой не снялась — кэш не стираю")
            return _sync_once_locked(client)
        ...
        try:
            ok = _sync_once_locked(client)
        except Exception:
            _restore_after_failed_reconcile(snapshot)
            raise
        if not ok:
            _restore_after_failed_reconcile(snapshot)
        return ok
...
def _restore_after_failed_reconcile(snapshot: str) -> None:
    """Вернуть базу из снимка, снятого перед очисткой (см. `reconcile`).

    Кэш уже стёрт, а наполнить его заново не вышло. Без отката человек остаётся с пустым
    журналом до появления связи — и это ровно та авария, о которой предупреждал докстринг
    словами «останется пустая база», не предлагая ничего взамен.

    ⚠️ Сам откат тоже может не удаться (файл занят, место кончилось). Молчать об этом
    нельзя: пустой журнал после сверки выглядит как «данные пропали», и связать это с
    сетью человек не сможет ничем.
    """
    if not snapshot:
        return
    from data.core import DBManager
    try:
        if DBManager.restore(snapshot):
            _log.warning("полный снимок с сервера не доехал — база возвращена из копии "
                         "%s, сверка повторится следующим циклом", snapshot)
        else:
            _log.error("полный снимок не доехал И копию %s вернуть не удалось — "
                       "локальные данные придётся восстанавливать вручную", snapshot)
    except Exception as e:      # noqa: BLE001 — откат не имеет права заслонить исходную беду
        _log.error("откат к копии %s не удался: %s", snapshot, e)
...
def _sync_once_locked(client) -> bool:
    ...
    #(docs/research/SYNC-RESEARCH-2026.md, шаг 5), но САМ КЛАСС отказа закрывается здесь и сейчас:
