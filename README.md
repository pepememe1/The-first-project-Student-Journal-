...
[![Версия](https://img.shields.io/badge/версия-Release_3.0-008080)](CHANGELOG.md)
...
Продукт находится в стадии **Release 3.0** и готовится к первому коммерческому внедрению
...
Монорепо: **десктоп** (PySide6) в корне, **сервер** (FastAPI) в [`server/`](server/),
...
## Один UI на две платформы

Онлайн-разделы десктопа (**мессенджер**, **модерация чатов**) — это тот же Vue-SPA,
встроенный в `QWebEngineView` (`ui/messenger_web.py`), а не отдельный порт на Qt: фича
пишется один раз и сразу работает на всех платформах. Оффлайн-способные разделы (журнал,
Вектор с локальным SQL) **остаются нативными** — веб-view сломал бы их без интернета.
