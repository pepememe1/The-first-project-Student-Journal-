...
Держим зелёными: **633 клиентских**, **734 серверных**, **37 веб-тестов**.
...
### Раскладка экрана (3.5.6)

Экран отдан данным, а не рамкам вокруг них:

- **Верхней полосы нет.** Всё, что в ней жило — бренд, роль, ФИО, статус, тема, выход —
  переехало по смыслу: бренд в шапку бокового меню, **карточка пользователя** (аватар,
  ФИО, логин, роль, статус, переключатель темы) — в его нижний угол, как в Discord и
  Teams, а выход — в самый низ «Настроек», подальше от ежедневных кнопок. На телефоне
  остаётся компактная полоса в 48 px: там боковое меню скрыто за бургером.
- **Заголовок страницы не дублируется.** Имя раздела и так подсвечено в меню; крупный
  повтор над контентом убран, остались только поясняющие подзаголовки.
- **Шторка «Вектора» накладывается поверх страницы**, а не раздвигает её: открытие
  помощника больше не перевёрстывает журнал под ним.
- **Вкладка «ИИ Помощник» показывает успеваемость** — кольцо общего среднего балла и
  полосу на каждый предмет рядом с перепиской (студенту; у преподавателя и админа
  «своего среднего» не существует).

## Автообновление десктопа

Установленная программа обновляется сама: при запуске фоном спрашивает у сервера
`/desktop/updates`, качает **дельту между сборками** (обычно единицы МБ вместо 48) и
ставит её при следующем запуске. Всё по обычному HTTPS на тот же домен — отдельных
портов и служб не нужно, вузовский фаервол такому не мешает.

- Правила («какая версия новее», имя патча, хеши) — в корневом `desktop_update.py`,
  общем для программы, сервера и сборщика: разъедутся — человек скачает то, что не
  подойдёт, и узнает об этом при запуске.
- Каждый шаг проверяется SHA-256: сам патч, результат его наложения и файл перед
  подменой. Не сошлось — обновление отменяется, рабочая версия остаётся на месте.
- Подмена только **переименованием** (`.exe` → `.old.exe`, новый → на место): Windows не
  даёт перезаписать запущенный файл, но даёт переименовать. Ставится строго на старте,
  до открытия окна — под работающим журналом файл не трогаем.
- Нет дельты, нет `zstandard`, нет сети — программа просто качает `.exe` целиком или
  остаётся на текущей версии. Это всегда рабочий запасной путь.

Выкладка новой сборки — один скрипт, он же считает патч и проверяет результат:

```bash
bash tools/publish_desktop_update.sh dist/GradeBookAI.exe
```

Патч публикуется, **только если он реально выгоден** (по умолчанию — выигрыш от 40 %):
onefile-сборка сжата, и дельта между двумя сжатыми файлами иногда почти равна самому
файлу. Скрипт сравнивает размеры и в таком случае оставляет только полную закачку,
вместо того чтобы называть экономией 45 МБ вместо 48.
