...
def users_to_students(users: list) -> list:
    ...
    for u in users:
        ...
        out.append({
            ...
            #prefs (тема оформления) тянем ТОЛЬКО в сторону pull → локальная запись,
            #чтобы при входе с этого ПК прочитать персональную тему пользователя.
            #Обратно (students_to_users/push) prefs НЕ отправляем — иначе push админа
            #мог бы затереть тему, которую пользователь только что задал через /me/prefs.
...
def users_to_teachers(users: list) -> dict:
    ...
    for u in users:
        ...
        out[u.get("full_name", "")] = {
            ...
            "prefs": u.get("prefs") or {},   #тема оформления (см. users_to_students)
            "updated_at": u.get("updated_at", ""),
        }
    return out
