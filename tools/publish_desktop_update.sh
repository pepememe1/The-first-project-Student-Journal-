...
# 2.1. ПОДПИСЬ ВЫПУСКА. До 10.09.2026 этого шага здесь НЕ БЫЛО: инструмент
#      tools/sign_release.py существовал, а звал его никто — наш класс «обещание без
#      вызывающего», только в инфраструктуре выкладки.
#      🔴 Отказ здесь ТИХИЙ и потому опасен: неподписанный манифест заливается успешно,
#      сервер отдаёт его с кодом 200, проверка «версия обновилась» (шаг 5) проходит — а
#      программа у человека молча перестаёт обновляться, и узнаем мы об этом через
#      недели. Поэтому подпись не «по возможности», а ворота.
python "$ROOT/tools/sign_release.py" --manifest "$WORK/updates/manifest.json"
# ⚠️ Проверяем ТЕМ ЖЕ кодом, которым проверяет программа (release_signature_ok), а не
#    наличием поля «sig»: подпись чужим ключом — это тоже поле. Урок validate-agents.py.
python "$ROOT/tools/sign_release.py" --verify "$WORK/updates/manifest.json" || {
  echo "ОСТАНОВЛЕНО: манифест не проходит собственную проверку подписи."
  echo "Ничего не залито. Разберитесь с ключом ДО выкладки."
  exit 5
}
# 🔴 НАЗВАННАЯ ГРАНИЦА ЭТИХ ВОРОТ (найдено разбором 10.09.2026).
# Проверка сверяет подпись с ключом ТОГО ДЕРЕВА, из которого запущена выкладка, — а не с
# тем, что вкомпилирован в выкладываемый .exe. По §8.1 сборку гоняют в ОТДЕЛЬНОМ
# worktree, то есть это два разных объекта, и связи между ними не проверяет ничто.
# Прочитать ключ из onefile-сборки нельзя: payload сжат. Поэтому здесь не молчаливая
# проверка, а громкое напоминание — и оно печатается ПЕРЕД заливкой, пока не поздно.
echo
echo "⚠️  ПОДПИСАНО ключом ЭТОГО дерева. Убедитесь, что .exe собран из дерева, где"
echo "    UPDATE_PUBLIC_KEYS содержит тот же ключ, иначе парк отвергнет обновление:"
python - "$ROOT" <<'PYKEY'
import sys, os
sys.path.insert(0, sys.argv[1])
import desktop_update as DU
for k in DU.UPDATE_PUBLIC_KEYS:
    print("      %s" % k)
PYKEY
echo
