...
#   3. ПОДПИСЫВАЕТ манифест (Ed25519) и проверяет подпись тем же кодом, что и клиент;
#   4. заливает патч, полный .exe и манифест в downloads/updates на VPS;
#   5. обновляет downloads/GradeBookAI.exe — кнопку «Скачать» на сайте;
#   6. проверяет, что сервер отдаёт новый манифест.
#
...
if [ -f "$KEY_PATH" ]; then
  ...
  # ВОРОТА: проверяем ТОЙ ЖЕ функцией, какой проверяет клиент (`release_signature_ok` внутри
  # `sign_release.py --verify` — и полный файл, и каждый патч). Сверять «есть ли поле sig»
  # бессмысленно — подпись чужим ключом это тоже поле. Проверка живёт в ОДНОМ месте: до
  # слияния 16.09.2026 у одной ветки она была вызовом инструмента, у другой — копией прямо в
  # скрипте, и две копии одной проверки однажды разошлись бы молча.
  python "$ROOT/tools/sign_release.py" --verify "$WORK/updates/manifest.json" || {
    echo "ОСТАНОВЛЕНО: подпись манифеста НЕ сходится с UPDATE_PUBLIC_KEYS. Ничего не залито:"
    echo "иначе парк молча остался бы без обновлений."
    exit 5
  }
  # 🔴 НАЗВАННАЯ ГРАНИЦА ЭТИХ ВОРОТ (разбор 10.09.2026, дополнено при слиянии 16.09.2026).
  # 1) Подпись сверяется с ключами ТОГО ДЕРЕВА, из которого запущена выкладка, а не с теми,
  #    что вкомпилированы в выкладываемый .exe (сборку гоняют в отдельном worktree, §8.1).
  # 2) Ключей в списке ДВА: их завели параллельно на двух машинах (10.09 и 12.09), а ворота
  #    выше пропускают подпись ЛЮБЫМ из них. Сборки 3.9.7, уже стоящие у людей, знают ТОЛЬКО
  #    первый ключ списка — подпись вторым они отвергнут молча. Поэтому печатаем, КАКОЙ ключ
  #    подписал, и останавливаемся, если не первый. Осознанный переход на другой ключ —
  #    GB_ALLOW_NONPRIMARY_KEY=1, и только когда парк уже стоит на сборке с обоими ключами.
  SIGNER="$(python -c "
import io, json, sys
...
full = m.get('full') or {}
keys = DU.UPDATE_PUBLIC_KEYS
for k in keys:
    DU.UPDATE_PUBLIC_KEYS = (k,)
    if DU.release_signature_ok(m.get('version', ''), full.get('sha256', ''), full.get('sig', '')):
        print(k)
        break
DU.UPDATE_PUBLIC_KEYS = keys")"
  PRIMARY="$(python -c "
import sys; sys.path.insert(0, r'$ROOT')
import desktop_update as DU
print(DU.UPDATE_PUBLIC_KEYS[0] if DU.UPDATE_PUBLIC_KEYS else '')")"
  echo "-- манифест подписан ключом: ${SIGNER:-не опознан}"
  if [ "$SIGNER" != "$PRIMARY" ] && [ "${GB_ALLOW_NONPRIMARY_KEY:-}" != "1" ]; then
    echo "ОСТАНОВЛЕНО: подписано не первым ключом UPDATE_PUBLIC_KEYS ($PRIMARY)."
    echo "Сборки, уже стоящие у людей, знают только его и молча отвергнут это обновление."
    echo "Подпишите ключом, соответствующим первому (GB_SIGNING_KEY), либо, если парк уже"
    echo "на сборке с обоими ключами, повторите с GB_ALLOW_NONPRIMARY_KEY=1."
    exit 5
  fi
