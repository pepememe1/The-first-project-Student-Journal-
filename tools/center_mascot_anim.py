...
#Куда смотреть. Один и тот же набор файлов лежит в двух местах: для десктопа (бандлится
#в .exe) и для веба (раздаётся сайтом и уезжает в мобильное приложение через OTA).
TARGETS = [
    os.path.join("emotions", "анимации"),
