...
def main() -> int:
    ...
    if orphan_routes:
        # 🔥 РАНЬШЕ ЭТОТ СПИСОК СЧИТАЛСЯ И НЕ ПЕЧАТАЛСЯ. Инструмент честно сообщал
        # «сервер никто не зовёт: 23» и не говорил, КОГО именно, — то есть находил наш
        # самый частый класс дефекта («обещание без вызывающего») и молчал о находке.
        # Поймано 25.08.2026, когда искали, почему в беседу нельзя добавить участника:
        # ручка `POST /chats/{}/members` существовала и работала, а в контракте клиента
        # её не было вовсе. Она была в этом списке всё время.
        #
        # ⚠️ Не всякая ручка здесь — дефект. Своих потребителей ЗАКОННО не имеют:
        #   • то, что зовёт ДЕСКТОП или мобилка, а не веб (`/sync/*`, `/desktop/updates`,
        #     `/app/updates`) — разбор кода Python-клиента мост не делает;
        #   • служебное (`/health`, отдача SPA);
        #   • то, что зовёт САМ сервер или внешняя система.
        # Поэтому список — повод посмотреть глазами, а не приговор.
        print("\n── ручки без потребителя в вебе (посмотреть глазами) ──")
        for r in sorted(orphan_routes, key=lambda x: (x["file"], x["path"])):
            where = r["file"].replace("server/app/routers/", "")
            print(f"  {r['verb']:6} {r['path']:52} {where}:{r['fn']}")
