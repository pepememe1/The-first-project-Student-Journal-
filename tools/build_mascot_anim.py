...
Зачем WebP: анимированный WebP с альфа-каналом играет во всех браузерных движках, на
которых мы живём, — Chromium сайта, Capacitor WebView на мобилке и системный Edge
(WebView2) в окне программы. Значит один файл на состояние покрывает все платформы, без
per-platform вариантов.
⚠️ Раньше здесь третьим пунктом стоял «Qt на десктопе (QMovie/QImageReader)» — Qt в
проекте нет, десктоп показывает ту же SPA. Практическое следствие: файлы нужны ТОЛЬКО в
`web/public/mascot/anim/`. Копия в `emotions/анимации/` (для нативных экранов) больше
никем не читается — новые состояния туда класть незачем.
...
Состояния (чат/док Вектора):
  ...
  eyes_close ← «деф_закрыв»        закрывает глаза лапами (экран входа: набран пароль)
  eyes_open  ← «деф_закрыв» НАОБОРОТ  убирает лапы (пароль стёрли)

⚠️ Обратный проигрыш — ОТДЕЛЬНЫЙ файл, а не «тот же назад». Ни браузер, ни Qt не умеют
отматывать анимированный WebP: reverse существует только на сборке. Зато оба направления
получаются из ОДНОГО ролика — Арине не нужно рисовать «открывание» вручную.
...
#eyes_close/eyes_open — тоже одноразовые: закрыл глаза и ДЕРЖИТ, пока пароль набран
#(иначе лапы дёргались бы вверх-вниз всё время ввода); открыл — и клиент сам переключает
#на покой. Без loop=1 «закрывание» крутилось бы по кругу.
LOOP = {"greeting": 1, "eyes_close": 1, "eyes_open": 1}
...
def main():
    ...
    states = {
        ...
        "eyes_close": load_keyed("деф_закрыв.mp4"),
    }
    #Обратный проигрыш того же ролика: лапы уходят от глаз. Копии кадров (а не ссылки) не
    #нужны — finalize() всё равно возвращает новые изображения.
    states["eyes_open"] = list(reversed(states["eyes_close"]))
    ...
    #Без стрелки/юникода: консоль Windows в cp1251 валила скрипт на самой последней
    #строке, когда все файлы уже были собраны — выглядело как провал сборки.
    print(f"ИТОГО: {total/1024:.1f} КБ, состояний {len(states)}, каталог: {out_dir}")
