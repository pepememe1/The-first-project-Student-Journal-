...
def updater_env(tmp_path, monkeypatch):
    ...
    **тест обязан задавать окружение САМ**. Здесь окружение включает и набор ключей.
    ...
    ⚠️ При слиянии с веткой 3.8.9 (16.09.2026) выбран не пустой набор, а КЛЮЧ СТЕНДА: стенд
    подписывает обновление сам. Так тесты подмены проходят и механику, и настоящий путь с
    подписью, а от домашнего каталога машины по-прежнему не зависят ни в одном режиме.
    """
