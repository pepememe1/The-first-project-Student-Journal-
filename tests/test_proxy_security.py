...
_ONLINE_ONLY_SUBSYSTEMS = {
    ...
    #Добавлено 20.09.2026 сверкой платформ: на сайте разделы работают, в программе
    #открывались пустыми. Причина у каждого своя и названа поимённо.
    "/web/admin/messenger": "модерация читает ПЕРЕПИСКУ, а она вне SYNC_MODELS (§5.4) — "
                            "в локальной копии её нет; префикс /web/messenger сюда не "
                            "достаёт, путь начинается с /web/admin/",
    "/web/admin/audit": "AuditEvent не синкуется: это след событий БОЕВОЙ машины. Без "
                        "пересылки проверка целостности шла бы по пустой локальной "
                        "таблице и отвечала «всё в порядке»",
    "/web/admin/invites": "StudentInvite не в SYNC_MODELS, а выданная ссылка обязана "
                          "существовать на бою — студент открывает её в браузере",
    "/web/admin/ai-config": "ключ и настройки ИИ живут в `config`, который НАМЕРЕННО "
                            "исключён из PUSH_SCOPE — правка из программы не долетела бы "
                            "до боя физически",
    "/web/admin/moderators": "заведение учётной записи: проверка «логин занят» и выдача "
                             "пароля обязаны идти на бою, иначе родится фантом, о котором "
                             "на сайте никто не знает",
    #4.0 — учётная запись.
    "/me/sessions": "AuthSession не синкуется: локально раздел показал бы сессии "
                    "ЛОКАЛЬНОГО сервера вместо боевых — правдоподобную ложь",
    "/me/contacts": "UserContact не в SYNC_MODELS: подтверждённая почта обязана "
                    "существовать на бою, туда уходит код подтверждения входа",
    "/me/password": "смена пароля обязана случиться на бою: локальная копия хеша "
                    "синком на сервер не поднимается (`_NEVER_BLANK`)",
