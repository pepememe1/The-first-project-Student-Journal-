...
test('про уже полученную ачивку вопрос ЗАДАЁТСЯ, но другими словами', () => {
  // 🔥 РАНЬШЕ ЗДЕСЬ БЫЛО ОБРАТНОЕ ПРАВИЛО, и его отменил живой отзыв (25.08.2026).
  // Полученные вырезались из `pending` целиком: смысл вопроса — «останьтесь, а то не
  // заберёте находку», а когда забирать нечего, он выглядел помехой. Снаружи это
  // читалось не как замысел, а как СЛОМАВШАЯСЯ функция: на новом аккаунте
  // предупреждение есть, на своём — нет. Влад так и сообщил, как баг.
  //
  // Теперь спрашиваем всегда, но реплика меняется с «не упустите» на «вы это уже
  // проходили, хотите ещё раз».
  assert.ok(!/function stillWorthIt/.test(store),
    'фильтр stillWorthIt вернулся — полученные снова молчат вместо второй реплики')
  assert.match(store, /const LEAVE_ASK_AGAIN = \{/, 'нет второго набора реплик')
  assert.match(store, /function alreadyOwned/, 'нечем отличить полученную от новой')
  assert.match(store, /owned\.value\.has\(aid\)/, 'признак не смотрит в список полученных')

  // Второй набор обязан покрывать РОВНО те же пасхалки, что и первый: иначе часть
  // людей получит безликую заглушку вместо реплики в голосе игры, и заметить это
  // можно только вручную поймав нужную пасхалку с нужным аккаунтом.
  const ids = (block) => new Set(
    [...block.matchAll(/^ {2}([a-z0-9_]+): \{/gm)].map((m) => m[1]))
  const cut = (name) => store.split(name)[1].split(String.fromCharCode(10) + '}')[0]
  const first = ids(cut('const LEAVE_ASK = {'))
  const again = ids(cut('const LEAVE_ASK_AGAIN = {'))
  assert.ok(first.size >= 7, `первый набор подозрительно мал: ${first.size}`)
  assert.deepEqual([...first].sort(), [...again].sort(),
    'наборы реплик разъехались: у кого-то нет варианта «уже было»')

  // И тексты обязаны РАЗЛИЧАТЬСЯ — иначе вторая реплика существует только на бумаге.
  for (const id of first) {
    const a = store.split('const LEAVE_ASK = {')[1].split(`  ${id}: {`)[1].split('  },')[0]
    const b = store.split('const LEAVE_ASK_AGAIN = {')[1].split(`  ${id}: {`)[1].split('  },')[0]
    assert.notEqual(a.trim(), b.trim(), `${id}: реплика «уже было» дословно повторяет первую`)
  }

  // pending больше ничего не вырезает: спрашиваем про оба канала.
  const pendingBody = store.split('const pending = computed(')[1].split('})')[0]
  assert.ok(!pendingBody.includes('stillWorthIt'), 'pending снова фильтрует полученные')

  // Роутер обязан ПЕРЕДАВАТЬ признак — иначе второй набор написан и не используется.
  assert.match(router, /leaveAsk\(easter\.pending,\s*easter\.pendingOwned\)/,
    'роутер зовёт leaveAsk без признака «уже получена» — второй набор мёртв')
})
