...
test('уходящая страница вынута из потока — иначе две страницы встают стопкой', () => {
  // 🔥 Влад, 25.08.2026: «переключался быстро между ИИ-помощником и сообщениями —
  // внизу экрана было видно верхнюю часть страницы сообщений».
  //
  // Это не подгрузка и не мигание, а прямое следствие ПРЕЖНЕЙ починки: когда убрали
  // `mode="out-in"` (он подвешивал переходы намертво), старая и новая страницы стали
  // жить в DOM одновременно — и вторая честно вставала обычным блоком ПОД первой на
  // те самые 150 мс. Вернуть `out-in` нельзя, поэтому уходящую вынимаем из потока.
  //
  // Проверяем ОБЕ половины: правило без `position: relative` на обёртке отсчитается от
  // окна и уедет под шапку, то есть починка развалится молча.
  const style = shell.split('<style scoped>')[1].split('</style>')[0]
  // ⚠️ Берём ВСЕ блоки `.fade-leave-active`, а не первый: первым идёт правило
  // прозрачности, и наивный split проверял бы не то (поймано на себе).
  const blocks = [...style.matchAll(/\.fade-leave-active\s*\{([^}]*)\}/g)].map((m) => m[1])
  assert.ok(blocks.length, 'правила .fade-leave-active нет вовсе')
  const joined = blocks.join(' ')
  assert.match(joined, /position:\s*absolute/,
    'уходящая страница снова в потоке — вернётся «видно верх следующей страницы»')
  assert.match(joined, /pointer-events:\s*none/,
    'уходящая страница ловит клики поверх новой')
  assert.match(style, /\.gb-pages\s*\{[^}]*position:\s*relative/,
    'нет системы координат — уходящая страница отсчитается от окна')
  assert.match(shell, /class="gb-pages"/, 'обёртка не помечена классом gb-pages')
})
