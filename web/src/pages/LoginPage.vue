...
// Наведение на Вектора: поза «думает» + облачко-совет. Советы крутятся по кругу.
const TIPS = [
  'Программа работает и без интернета: данные сохранятся локально и синхронизируются позже.',
  'Оценки, средний балл, долги и пропуски — всегда под рукой.',
  'Я беру цифры из реальных данных, а не выдумываю их.',
  'Тёмную тему можно включить по расписанию — вечером глазам легче.',
  'Расписание тянется прямо с портала ВСГУТУ.',
]
...
<template>
  <div class="relative flex min-h-full items-center justify-center overflow-hidden p-4">
    ...
    <!-- Сетка 1fr · auto · 1fr: карточка входа СТРОГО по центру экрана (боковые колонки
         равны), items-end — ноги Вектора и низы карточек на одной линии. -->
    <div class="relative z-10 grid w-full max-w-6xl grid-cols-1 items-end gap-x-6 gap-y-4 lg:grid-cols-[1fr_auto_1fr]">
