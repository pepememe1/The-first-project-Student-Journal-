...
/**
 * Цвет «плашки профиля» по id пресета из prefs.profile_color.
 *
 * Единая точка для всех мест, где показывается карточка пользователя (профиль, шапка
 * чата, боковая панель, «О беседе»). ВАЖНО: подставляем только цвет ИЗ СПИСКА пресетов —
 * произвольную строку из prefs в CSS не пускаем. Неизвестный/пустой id — акцент темы.
 */
export function profilePlate(colorId) {
  return PRESET_BY_ID[colorId]?.accent || 'var(--gb-accent)'
}
