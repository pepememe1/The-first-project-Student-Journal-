    ...
    <div class="flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      <main class="app-canvas min-h-0 flex-1 overflow-y-auto" style="padding-bottom: env(safe-area-inset-bottom)">
        ...
        <div :class="[embed ? 'h-full p-0' : 'mx-auto max-w-[1700px] p-3 sm:px-4 sm:py-3.5',
          ...
          <!-- ⚠️ Живой отзыв: «вкладка иногда не прогружается, помогает только F5» —
               по построению это переход `mode="out-in"`: Vue не покажет НОВУЮ страницу,
               пока не получит подтверждение, что СТАРАЯ закончила `leave`-переход, а это
               подтверждение — событие `transitionend` из CSS. Если браузер его не
               пришлёт (типичная причина — фоновая/невидимая вкладка перед переключением:
               часть браузеров придерживает рендер CSS-переходов у неактивных вкладок),
               Vue ждёт вечно — новая страница физически не может появиться, F5 обходит
               эту логику целиком и рисует страницу заново без всякого перехода. `:duration`
               переводит Vue на явный JS-таймер вместо ожидания браузерного события —
               тот же визуальный fade, но переход гарантированно завершается сам. Диагноз
               по коду, не воспроизведён живым тестом (dev-browser вне работы, см. шапку
               файла CLAUDE.md) — если проблема останется, дальше смотреть сюда. -->
