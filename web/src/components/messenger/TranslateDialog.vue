...
const FIELDS = [
  { key: 'incoming_from', label: 'Язык, с которого переводить входящие', auto: true },
  { key: 'incoming_to', label: 'Язык, на который переводить входящие', auto: false },
  { key: 'outgoing_from', label: 'Язык, с которого переводить ваши сообщения', auto: true },
  { key: 'outgoing_to', label: 'Язык, на который переводить ваши сообщения', auto: false },
]
</script>
    ...
    <div class="w-full max-w-lg rounded-2xl border border-border bg-card p-5 shadow-xl">
      <div class="mb-4 flex items-center justify-between gap-3">
        <h2 class="flex items-center gap-2 font-title text-lg font-extrabold text-text">
          <Languages class="size-5 text-accent" />Перевод
        </h2>
        <button type="button" class="text-text3 transition-colors hover:text-text"
                aria-label="Закрыть" @click="$emit('close')">
          <X class="size-5" />
        </button>
      </div>
      ...
      <div class="flex max-h-[60vh] flex-col gap-4 overflow-y-auto">
        <label v-for="f in FIELDS" :key="f.key" class="block">
          ...
          <select :value="tr.prefs[f.key]"
            ...
            <option v-if="f.auto" value="auto">Определить язык</option>
            <option v-for="l in tr.languages" :key="l.code" :value="l.code">{{ l.name }}</option>
          </select>
        </label>

        <ToggleRow label="Автоперевод"
                   hint="Переводить ваши сообщения перед отправкой. Текст в поле ввода заменится
                         на перевод — вы увидите его до того, как нажмёте «отправить»."
        ...
        <div class="flex items-start gap-2.5 rounded-lg border border-border bg-card2 p-3 text-xs text-text3">
          ...
          <p>
            Переводит та ИИ-модель, которую подключил администратор колледжа. Если это
            облачная модель — текст сообщения уходит ей. Ничего не переводится само:
            входящие только по кнопке, исходящие — пока включён автоперевод.
          </p>
        </div>
