...
const PERM_LABELS = {
  ...
  manage_roles: 'Выдавать роли и создавать новые',
  cmd_mute: 'Команда /mute',
...
async function saveRole() {
  ...
  try {
    ...
    return role
  } catch (e) { toast.error(e?.response?.data?.detail || 'Не удалось сохранить роль'); return null }
...
async function removeRole(r) {
  if (!window.confirm(`Удалить роль «${r.name}»? Участники с ней вернутся к обычной роли.`)) return
  try { await messengerApi.deleteRole(m.activeId, r.id); await load() }
  catch (e) { toast.error(e?.response?.data?.detail || 'Не удалось удалить роль') }
}

async function assign(roleId) {
  const ok = await m.setMemberRole(props.userId, { customRoleId: roleId })
  if (ok) { toast.success('Роль назначена'); emit('close') }
  else toast.error('Не удалось назначить роль')
}
async function resetToDefault() {
  const ok = await m.setMemberRole(props.userId, { role: 'member' })
  if (ok) { toast.success('Роль сброшена'); emit('close') }
  else toast.error('Не удалось сбросить роль')
}
</script>
  ...
  <div class="fixed inset-0 z-[60] grid place-items-center p-4" style="background: var(--gb-overlay)"
    ...
    <div class="flex max-h-[85vh] w-full max-w-sm flex-col rounded-xl border border-border2 bg-card shadow-card">
      <div class="flex items-center justify-between border-b border-border p-4">
        <h3 class="font-title text-base font-bold text-text">Роль · {{ userName || 'участник' }}</h3>
      ...
      <div class="min-h-0 flex-1 overflow-y-auto p-4">
        ...
        <template v-else-if="!showEditor">
          <button type="button" @click="resetToDefault"
            ...
            Без кастомной роли (обычный участник)
          </button>

          <p v-if="roles.length" class="mb-1 text-[11px] uppercase tracking-wide text-text3">Роли беседы</p>
          <div v-for="r in roles" :key="r.id"
            ...
            <button type="button" class="min-w-0 flex-1 text-left" @click="assign(r.id)">
              ...
              <div class="truncate text-[11px] text-text3">
                {{ (r.permissions || []).map(p => PERM_LABELS[p] || p).join(', ') || 'без особых прав' }}
              </div>
            </button>
            <button type="button" title="Изменить" class="shrink-0 text-text3 hover:text-accent" @click="startEdit(r)">✎</button>
            <button type="button" title="Удалить" class="shrink-0 text-text3 hover:text-red" @click="removeRole(r)">✕</button>
          </div>

          <p class="mb-1 mt-3 text-[11px] uppercase tracking-wide text-text3">Шаблоны</p>
        ...
        <template v-else>
          <label class="mb-3 block"><span class="mb-1 block text-tiny uppercase text-text3">Название роли</span>
          ...
          <p class="mb-1 text-[11px] uppercase tracking-wide text-text3">Права</p>
          ...
          <div class="mt-4 flex justify-end gap-2">
            <button type="button" @click="showEditor = false" class="rounded-md px-3 py-1.5 text-sm text-text3 hover:bg-bg2">Отмена</button>
