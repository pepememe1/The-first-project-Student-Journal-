...
//Набор статусов — общий (config/status.js): раньше он жил копией в каждом компоненте.
const KINDS = STATUS_KINDS
...
const current = computed(() => KINDS.find(k => k.kind === myStatus.value.kind) || KINDS[0])
...
<template>
  <div class="relative">
    ...
    <div v-if="open" class="absolute left-0 top-full z-20 mt-1 w-56 rounded-lg border border-border2 bg-card p-1.5 shadow-card">
      <button v-for="k in KINDS" :key="k.kind" type="button" @click="pick(k.kind)"
