...
const PRESETS = [
  { label: 'Состояние службы', cmd: 'systemctl status gradebook --no-pager -l | head -20' },
  { label: 'Последние ошибки', cmd: 'journalctl -u gradebook -p err -n 40 --no-pager' },
  { label: 'Журнал сервера', cmd: 'journalctl -u gradebook -n 60 --no-pager' },
  { label: 'Перезапустить сервер', cmd: 'systemctl restart gradebook && sleep 3 && systemctl is-active gradebook' },
  { label: 'Перезапустить Caddy', cmd: 'systemctl restart caddy && systemctl is-active caddy' },
  { label: 'Место на диске', cmd: 'df -h' },
  { label: 'Что занимает место', cmd: 'du -sh /root/gb-deploy/* 2>/dev/null | sort -rh | head -15' },
  { label: 'Сертификат сайта', cmd: 'journalctl -u caddy -n 30 --no-pager | grep -i cert' },
]
  ...
  try {
    ...
    if (e?.response?.status === 409) {
      ...
      history.value.push({
        ...
        err: e?.response?.data?.detail || 'Не удалось выполнить команду',
      })
    }
  } finally {
    busy.value = false
  ...
  <div class="flex flex-col gap-3">
    ...
    <div ref="logBox"
      ...
      <p v-if="!history.length" class="text-text3">
        Команда выполняется отдельным сеансом ssh: «cd» не запоминается между командами,
        а интерактивные программы (top, nano) работать не будут.
      </p>
      <div v-for="(h, i) in history" :key="i" class="mb-3 last:mb-0">
        ...
        <p class="mt-1 text-tiny" :class="h.ok ? 'text-text3' : 'text-red'">
          код {{ h.code }}<template v-if="h.seconds"> · {{ h.seconds }} с</template>
        </p>
      </div>
    </div>
    ...
    <div v-if="pending"
      ...
      <p class="flex items-start gap-2 text-sm text-text2">
        ...
        <span>Команда может необратимо повредить сервер или данные. Проверьте путь
              ещё раз — восстановить будет нечем, кроме резервной копии.</span>
      </p>
      <pre class="overflow-x-auto rounded-sm bg-card px-2 py-1.5 font-mono text-xs text-red">{{ pending }}</pre>
      <div class="flex gap-2">
        <AppButton variant="red" :disabled="busy" @click="send(true)">Всё равно выполнить</AppButton>
    ...
    <form class="flex gap-2" @submit.prevent="send(false)">
      <input v-model="command" :disabled="busy" spellcheck="false"
             placeholder="Команда, например: systemctl status gradebook"
      ...
      <AppButton type="submit" :disabled="busy || !command.trim()">
        <Play class="mr-1.5 inline size-4" />{{ busy ? 'Выполняем…' : 'Выполнить' }}
      </AppButton>
    </form>
  </div>
</template>
