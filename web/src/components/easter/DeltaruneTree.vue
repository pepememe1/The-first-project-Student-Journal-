...
//
// 🔥 СМОТРИМ НА АЧИВКУ, А НЕ НА ПАМЯТЬ УСТРОЙСТВА (24.08.2026). Через это прошли два
// неверных варианта подряд:
//   1. общий ключ `gb.egg.tree` на всё устройство — на общем компьютере колледжа
...
//   2. тот же ключ, но с логином и со стиранием при выходе — и тогда у человека, у
//      которого яйцо ДАВНО есть, диалог про его получение начинался заново. Влад
//      поймал это на своём аккаунте.
// Правильный источник правды один, и он на сервере: есть ачивка — яйцо уже взято.
// `owned` подгружается оболочкой при входе; не успел приехать — покажем полный диалог
// и повторный `claim` честно ответит «уже была». Это безобидная сторона ошибки.
//
// ⚠️ Правило шире дерева: у ЛЮБОЙ пасхалки, где первый и последующие показы
// отличаются, признаком «уже было» обязана быть АЧИВКА, а не отметка на устройстве.
const taken = computed(() => easter.owned.has('deltarune_egg'))
const lines = ref([])
const dlg = ref('')
