  ...
  <div class="fixed inset-0 z-[93] grid place-items-center p-4" style="background:var(--gb-overlay)"
    ...
    <div v-if="!done" class="flex flex-col items-center gap-3">
      <div class="relative">
        ...
        <!-- Координаты ПЕРЕМЕРЕНЫ по файлу от 30.08.2026: пламя x 17–19 %, y 19–24 %.
             Берём с запасом — промахиваться по пиксельному огоньку человеку не должно
             быть обидно. ⚠️ Меняешь рисунок — перемеряй: промах здесь не даёт ни ошибки,
             ни следа в консоли, пасхалка просто молча не срабатывает. Ровно это и
             случилось при замене рисунка: у новой композиции свеча выше и левее, старый
             бокс (x 20–36 %, y 30–46 %) попал на футболку, и ачивка `portal_cake` стала
             недостижимой — при живой сцене и зелёных тестах. -->
        <button type="button" @click="blow" aria-label="Задуть свечу"
                class="gb-candle absolute left-[10%] top-[13%] h-[16%] w-[16%] cursor-pointer
    ...
    <div v-else class="flex flex-col items-center gap-3">
      <!-- ⚠️ «-2» В ИМЕНИ — НЕ ОПЕЧАТКА, А ЕДИНСТВЕННЫЙ СПОСОБ ДОСТАВИТЬ ПРАВКУ.
           ...
           отдаёт правильное. -->
      <!-- 🔥 СТЕНА ПОД РИСУНКОМ ОБЯЗАТЕЛЬНА, а не украшение. Силуэт на картинке ЧЁРНЫЙ,
           а сцена лежит на тёмном оверлее — без светлой подложки от рисунка видны
           только красные надписи и торт, а сам маскот исчезает. Ровно это и выглядело
           как «всё чёрное».
           Светлая она ещё и по существу: «торт — это ложь» в оригинале нацарапано на
           стене, и бетон читается правильнее любого другого фона.
           ⚠️ Подложка НАМЕРЕННО не зависит от темы: это отдельный визуальный мир на
           несколько секунд, и в тёмной теме он обязан остаться светлым — иначе мы
           вернёмся к чёрному по чёрному. -->
      <div class="gb-wall rounded-lg p-3 shadow-2xl">
        <img src="/easter/img/cake-lie-4.webp" alt="Торт — это ложь"
...
<style scoped>
/* Бетонная стена, на которой нацарапаны надписи. Тёплый серый, чуть темнее к краям —
   иначе прямоугольник читается как лист бумаги, а не как стена. */
.gb-wall {
  background:
    radial-gradient(115% 85% at 50% 0%, #ece7dd 0%, #ddd6c9 58%, #c9c1b2 100%);
}
