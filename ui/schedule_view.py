...
class ScheduleView(QWidget):
    ...
    def _build(self):
        ...
        self._btn_w1 = QPushButton("I неделя")
        self._btn_w2 = QPushButton("II неделя")
        for b, wk in ((self._btn_w1, 1), (self._btn_w2, 2)):
            b.setCheckable(True)
    ...
    def _load_from_cache(self):
        self.snapshot = sched.load_cached()
    ...
    def _resolve_identity(self):
        ...
        по их роли, админ выбирает свободно."""
        names = self._entities()
        if not names:
            return
        chosen = ""
        #1) ранее сохранённый личный выбор (per-account)
    ...
    def _on_entity_changed(self):
        ...
        #запоминаем личный выбор студента/преподавателя, чтобы не выбирать каждый раз
        if self.role in ("student", "teacher") and self.entity:
    ...
    def _sync_week_buttons(self):
        cur = sched.current_week_parity()
        for b, wk in ((self._btn_w1, 1), (self._btn_w2, 2)):
            active = (wk == self.week)
            b.setChecked(active)
            #текущая по календарю неделя помечается «(сейчас)»
            base = sched.week_label(wk)
            b.setText(base + ("  • сейчас" if wk == cur else ""))
