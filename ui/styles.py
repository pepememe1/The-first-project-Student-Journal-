...
#КОНСТАНТЫ ─────────────────────────────────────────────────────────────────────
#⚠️ Сами значения ПЕРЕЕХАЛИ в `data/defaults.py`: это данные предметной области, а не
#оформление, и пока они лежали здесь, слой данных импортировал слой интерфейса
#(`data/utils.py` начинался с `from styles import ...`). Здесь остаётся ре-экспорт —
#он ничего не стоит и не ломает возможных читателей, которые берут их отсюда по
#исторической привычке. Новый код должен брать их из `defaults`.
from defaults import DEFAULT_GROUPS, DEFAULT_SUBJECTS  # noqa: F401,E402
