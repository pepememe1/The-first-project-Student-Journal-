...
• НАТИВНЫМ ОСТАЁТСЯ ТОЛЬКО ТО, ЧЕГО В БРАУЗЕРЕ НЕТ В ПРИНЦИПЕ — «Инструменты ПК» админа
  (хостинг сервера, обслуживание БД). Они доступны странице через `js_api` (см. `_DeskAPI`),
  а не отдельной панелью поверх: накладки над страницей мы не заводим.
...
class _DeskAPI:
    ...
    def open_desktop_tools(self):
        """«Инструменты ПК» админа: хостинг сервера, обслуживание базы этого компьютера."""
        try:
            import desktop_tools
            desktop_tools.open_tools()
            return {"ok": True}
        except Exception as e:      # noqa: BLE001 — страница обязана получить ответ
            _LOG.warning(f"[webview2] инструменты не открылись: {e}")
