...
def _pixmap_from_dataurl(data_url: str) -> QPixmap:
    """data:URL → картинка. Пустой QPixmap, если это не она.

    ⚠️ С 3.7 в `prefs.avatar` может лежать НЕ картинка, а ССЫЛКА на гифку с CDN Klipy
    (аватарку-гифку выбирают в веб-интерфейсе — он же интерфейс программы, §11).
    Здесь такие значения сознательно НЕ грузим: качать картинку из сети в потоке
    интерфейса нельзя, а нативная Qt-оболочка — запасной путь, которого нет в релизной
    сборке. Пустой QPixmap вызывающий заменит обычной заглушкой с буквами имени.

    Проверяем именно СХЕМУ, а не наличие запятой, как было раньше: запятая законно
    встречается и в обычном URL, и тогда сюда прилетал бы кусок ссылки на разбор
    base64 — работало это лишь потому, что разбор падал и исключение глоталось."""
    pix = QPixmap()
    try:
