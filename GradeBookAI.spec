...
#  • subjects.py/subjects.json УДАЛЕНЫ 17.08.2026: список предметов берётся с портала
#    ВСГУТУ, а в программу приезжает синком в зеркальную копию local_app.db.
...
# python-docx (Word-экспорт журнала/ведомости) — импортируется лениво серверным
# server/app/docx_export.py. ⚠️ Здесь стояло `data/exports.py` — мёртвая копия того же
# модуля, оставшаяся от нативного журнала; удалена 01.09.2026 (ноль вызывающих в продукте).
