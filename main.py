...
def _apply_pending_update():
    """Установить обновление, скачанное в прошлый запуск (если есть), и попросить перезапуск.

    Обновления — не условие работы журнала: любая ошибка здесь глушится, программа просто
    продолжает запускаться на текущей версии. Печатаем в консоль, а не показываем окно:
    на этом этапе ни Qt, ни WebView2 ещё не подняты."""
    try:
        from core import APP_VERSION
        import updater
        if updater.apply_pending(APP_VERSION):
            print("Установлено обновление GradeBookAI. Запустите программу ещё раз.")
            raise SystemExit(0)
    except SystemExit:
        raise
    except Exception as e:      # noqa: BLE001 — обновление не имеет права мешать запуску
        print(f"[update] пропускаю установку обновления: {e}")


def _start_update_check():
    """Фоновая проверка новых версий (ничего не подменяет — только качает про запас)."""
    try:
        from core import APP_VERSION
        import app_settings
        import updater
        ...
        if url:
            updater.start_background_check(url, APP_VERSION)
    except Exception as e:      # noqa: BLE001
        print(f"[update] проверка обновлений недоступна: {e}")
...
def main():
    ...
    #Проверка новых версий — ФОНОМ, ничего не подменяя: скачанное встанет при следующем
    #запуске. Ошибки внутри не поднимаются наверх (см. updater.start_background_check):
    #недоступный сервер обновлений не повод не пустить человека в журнал.
    _start_update_check()
