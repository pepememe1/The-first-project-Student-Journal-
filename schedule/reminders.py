...
Как подключать потом (план ОБНОВЛЁН — прежний ссылался на `main_window` с QTimer и на
`vector.engine`, а ни того, ни другого в проекте больше нет: Qt-оболочка удалена, и
нативного Вектора на десктопе не осталось):
  • тикать раз в минуту — не из десктопа, а на СЕРВЕРЕ, попутно с уже существующим
    `GET /me/events` (тот же приём, что у `_fire_due_reminders` и уборки хранения: не
