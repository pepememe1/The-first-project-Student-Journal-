...
⚠️ ЧТО ИЗ ЭТОГО API РЕАЛЬНО ЗОВУТ СЕГОДНЯ (важно, чтобы не принять список выше за
описание живого кода). Со стороны СЕРВЕРА — `parser` и `model.strip_subgroup_tag`. Со
стороны ДЕСКТОПА — ровно две функции store: `subjects_for_group` (data/utils.py) и
`subjects_all` (subjects.py). Всё остальное (load_cached/save/get_identity/guess_*/
group_schedule/current_week_parity/week_label) вызывалось нативным экраном расписания,
которого нет с удаления Qt: сегодня расписание человеку показывает SPA, а данные для неё
собирает `server/app/schedule_web.py` — со СВОИМ кэшем и СВОИМ расчётом чётности недели.
Функции оставлены рабочими и покрытыми тестами намеренно (парсер и модель к ним не
привязаны, а стоимость хранения нулевая), но считать их «тем, что крутится у людей»
нельзя.
