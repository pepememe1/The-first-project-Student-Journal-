...
def current_week_parity(d: date | None = None) -> int:
    ...
    🔥 ИСПРАВЛЕНО (3.6.9). Раньше здесь стояло `js_getday` от САМОЙ ДАТЫ, и чётность
    менялась ВНУТРИ одной календарной недели по два-три раза:

        2026-02-02 Пн → I,  02-03..02-06 Вт-Пт → II,  02-07 Сб → I,  02-08 Вс → II

    Причина: это классическая JS-идиома «номер недели года», в которой первое
    слагаемое обязано быть днём недели 1 ЯНВАРЯ. С днём недели текущей даты
    числитель растёт на ДВА в сутки (растут оба слагаемых сразу), и «неделя»
    сменяется каждые ~3.5 дня, а в воскресенье откатывается назад.

    Баг поймал не тест на конкретные значения (контракт из 30 дат был зелёным —
    он сверял Python с Java, а обе стороны были одинаково неправы), а тест на
    ИНВАРИАНТ: внутри недели чётность обязана быть постоянной.

    ⚠️ Комментарий раньше обещал «точную реплику портального week()». Сверить это с
    порталом НЕ УДАЛОСЬ: страницы отдают уже готовый HTML, скрипта на них нет.
    Если окажется, что портал считает по-старому — мы разойдёмся с ним на неделю,
    и это будет видно сразу; тогда решение принимать заново, а не молча откатывать.
    """
    if d is None:
        d = date.today()
    one_jan = date(d.year, 1, 1)
    number_of_days = (d - one_jan).days
    #Python: Пн=0..Вс=6; JS getDay(): Вс=0..Сб=6 → сдвигаем. Берём день недели
    #1 ЯНВАРЯ — он постоянен весь год, поэтому числитель растёт ровно на 1 в сутки.
    jan1_js_getday = (one_jan.weekday() + 1) % 7
    result = math.ceil((number_of_days + jan1_js_getday + 1) / 7)
