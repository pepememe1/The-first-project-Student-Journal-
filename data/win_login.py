...
🔴 ФИЧА СЕЙЧАС НЕДОСТУПНА ЧЕЛОВЕКУ: этот модуль не импортирует НИКТО, кроме тестов.
Включали её галочкой на нативном экране входа, а его нет с удаления Qt; веб-форма входа
(`desktop/local_api.py::install_login_bridge`) о ней не знает. То есть пароль сейчас не
сохраняется НИГДЕ и ни у кого — инвариант «пароль не хранится» (§4.7) выполняется без
исключений, хотя документация продолжает описывать исключение как действующее.
⚠️ Прежде чем возвращать фичу в веб-форму входа, перечитать предупреждение ниже про
общий ПК колледжа: там она опасна ровно так же, как была.

Задача: перестать вводить логин и пароль журнала на своём компьютере. Персистентная
сессия по JWT это решает лишь частично — токен живёт жёстко 5 часов (§6 внутренних правил), после
...
⚠️ ЭТО ОСОЗНАННОЕ ИСКЛЮЧЕНИЕ из инварианта «пароль не хранится» (§4.7 внутренних правил): по
