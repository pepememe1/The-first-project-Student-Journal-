...
def connect(path: str, key: str = "", timeout: int = 10):
    ...
    """
    conn = None
    #🔥 ФАЙЛ ЕЩЁ ОТКРЫТЫЙ — ОТКРЫВАЕМ ЕГО БЕЗ КЛЮЧА, И ЭТО НЕ ПОБЛАЖКА.
    #Между запуском программы и миграцией (`encrypt_in_place` в `DBManager.init`) есть
    #окно, в котором ключ уже есть, а база ещё не зашифрована: в это окно к ней успевает
    #обратиться чтение адреса сервера. Попытка открыть открытый файл С ключом даёт
    #«file is not a database», и в логе ПЕРВОГО старта после обновления появлялись две
    #строки ошибок на ровном месте (поймано живым прогоном автообновления 29.08.2026).
    #Само оно лечилось через секунду, но такой шум прячет настоящие сбои — а именно по
    #ним потом ищут причину. Ключ при этом никуда не девается: миграция отработает
    #следующим шагом и файл станет зашифрованным.
    if key and is_plaintext(path):
        key = ""
    if key:
        try:
