...
def _groups_read_raw() -> list:
    ...
    try:
        ...
        #Чем опасен пустой список ИМЕННО здесь: `set_groups(stamp=True)` строит надгробия
        #сравнением нового списка с ТЕМ, ЧТО СЕЙЧАС В БАЗЕ (`_merge_list_tombstones`).
        #Прочитали «пусто» — сравнивать не с чем, надгробий не будет вовсе, и удаление
        #группы НЕ УЕДЕТ НА СЕРВЕР НИКОГДА (удаление ходит только надгробием). Ни ошибки,
        #ни строки в логе: админ удалил группу, у себя её не видит, а на сайте она живёт.
