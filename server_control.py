...
🔴 ВЫЗЫВАЮЩИХ В ПРОДУКТЕ НЕТ — ПРОВЕРЕНО, А НЕ ПРЕДПОЛОЖЕНО. Ни один .py, ни один .vue
не импортирует этот модуль; единственный, кто его трогает, — `tests/test_server_control.py`.
Кнопки, которые всё это запускали, жили в НАТИВНОЙ админ-панели Qt, а её в проекте нет
(удалена, лежит в WORK/GB-legacy-qt). В SPA раздел «Сервер» — это ДРУГОЕ:
`desktop/server_admin.py` + `AdminServer.vue`, управление БОЕВЫМ VPS по SSH, а не
подъём сервера на этом компьютере.

⚠️ ЧТО ИЗ ЭТОГО СЛЕДУЕТ ПРАКТИЧЕСКИ, помимо мёртвого веса:
  • `main.py --run-server` жив и по-прежнему поднимает uvicorn на 0.0.0.0 — но запустить
    его теперь может только человек руками из командной строки, продукт этого не
    предлагает нигде;
  • докстринг `data/updater.py` объясняет запрет HTTP у автообновления через сценарий
    «наша же кнопка „этот ПК как сервер для ЛВС“ раздаёт клиентам http://<ip>:8000».
    Кнопки нет — но заслонку СНИМАТЬ НЕЛЬЗЯ: адрес сервера человек и так задаёт руками,
    а обновление по открытому каналу это исполнение чужого кода.
Решение о судьбе модуля (вернуть точку входа в SPA либо удалить целиком, как поступили
с Qt) — за Ярославом; молча удалять работающую фичу уборкой кода неправильно.

Зачем он задумывался. В небольшом колледже один ПК может быть и рабочим местом
администратора, и сервером синхронизации. Модуль позволял из админ-панели десктопа:
