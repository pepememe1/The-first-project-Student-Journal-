...
Идея Ярослава по мотивам «Мистер Робот»; разбор и требования — `docs/done/PLAN-HONEYPOT.md`.
...
def test_the_hit_lands_in_the_persistent_audit_log(client, monkeypatch):
    ...
    `docs/done/PLAN-HONEYPOT.md` требует `audit_events`, и требует не из аккуратности:
...
def test_the_raw_address_never_reaches_the_permanent_log(client, monkeypatch):
    ...
    `docs/done/PLAN-HONEYPOT.md`: «срок хранения записей — иначе бессрочное накопление ПДн
