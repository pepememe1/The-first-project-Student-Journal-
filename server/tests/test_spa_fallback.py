...
def test_the_site_itself_still_opens(client):
    """Обратная сторона: обычные адреса сайта обязаны по-прежнему отдавать страницу.

    Без этой проверки достаточно расширить список префиксов до «всего», и
    клиентский роутинг Vue перестанет работать целиком — а заметят это люди, а не мы.
    """
    for path in ("/login", "/dashboard", "/messenger/42"):
        r = client.get(path)
        assert r.status_code == 200, f"{path} перестал отдавать сайт"
