...
#Конкурентность держим НИЖЕ пула соединений SQLAlchemy (5+10=15): при входе каждый
#запрос держит соединение всё время дорогого хеша пароля, и при сотнях одновременных
#пул исчерпывается (TimeoutError) — это отдельный вывод для прода (PostgreSQL/больше пул).
#Сервер и так упирается в CPU хеша, поэтому 12 достаточно, чтобы загрузить его без ошибок.
CONCURRENCY = 12
