...
удобства подключить `ui/server_admin.py` в `app.main`, и любая дыра в веб-админке
...
def test_prod_server_has_no_command_execution(client):
    ...
    Если этот тест упал — кто-то подключил ui/server_admin.py к серверному приложению.
