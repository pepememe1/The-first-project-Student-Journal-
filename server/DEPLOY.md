...
- Получают `.exe`, в `api_config.json` рядом с ним — адрес сервера
  (`{"api_url":"http://<IP-хоста>:8000"}`), или админ зашивает его в сборку
  (`app_settings.DEFAULT_API_URL`). Больше ничего: вошёл логином/паролем — данные
  подтянулись. **К PostgreSQL клиенты не подключаются** — только к API.
...
2. в проге: админ-панель → вкладка «Сервер» → «Адрес сервера»;
...
Адрес сервера остаётся настройкой (`api_config.json` / `DEFAULT_API_URL`) — заказчик
подставляет свой домен, в коде ничего не зашито.
