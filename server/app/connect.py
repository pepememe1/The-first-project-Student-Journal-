...
def host_device_id() -> str:
    """device_id ПК-хоста — читается из окружения GRADEBOOK_HOST_DEVICE_ID.

    ⚠️ Переменную выставлял `server_control.py` (модель «сервер на ПК преподавателя»),
    удалённый 15.08.2026. Сегодня её задаёт вручную администратор боевой машины в `.env`,
    и смысл прежний: хост, с которого одобряют остальных, обязан проходить барьер сам —
    иначе получился бы замкнутый круг (одобрить себя было бы некому). На IP не полагаемся:
