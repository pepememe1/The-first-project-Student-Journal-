...
def default_term() -> tuple:
    ...
    Календарь:
      ...
      • 25 авг – 31 авг   → уже новая осень Y/Y+1 (сем1), с запасом до 1 сентября.

    ⚠️ ГРАНИЦА ЛЕТА ПЕРЕСМОТРЕНА В 3.6, и вот почему. Раньше новый учебный год начинался
    1 ИЮЛЯ: так занятия, заведённые летом на сентябрь, сразу попадали в текущий термин.
    Но от термина считается ещё и КУРС студента (`study_hours.course_and_semester`), а
    курс группы независимо показывает портал ВСГУТУ по своему индексу расписания — и всё
    лето эти два источника расходились ровно на год: К74/1 по расписанию второй курс, по
    нашей дате уже третий. Две разные правды об одном и том же на соседних экранах хуже,
    чем неудобство при подготовке к сентябрю.

    Поэтому граница — 25 АВГУСТА: учебный год переключается за неделю до 1 сентября
    (успеть завести расписание и часы), а не в разгар каникул. Прежняя польза почти
    сохранена: подготовка нового года как раз и идёт в конце августа. Кому нужно раньше —
    выбирает термин вручную селектором, это ОДИН осознанный клик вместо круглогодичного
    расхождения в курсе.
