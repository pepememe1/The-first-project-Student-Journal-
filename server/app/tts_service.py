...
ПДн в синтез не попадают: озвучиваем уже обезличенный ответ Вектора (имён там нет —
списки студентов идут флагом no_voice и не проговариваются, см. web.py).

Два логических голоса продукта → спикеры Silero:
  male   = aidar — полегче и мягче (Вектор — дружелюбный тигр, а не грубый бас);
  female = baya.
"""
import io
import os
...
def engines() -> list:
    """Список доступных движков синтеза (для UI/диагностики)."""
