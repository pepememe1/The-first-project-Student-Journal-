...
async def lifespan(app: FastAPI):
    ...
    #🎓 ПЕРЕВОД ГРУПП НА НОВЫЙ КУРС — АВТОМАТИЧЕСКИ (06.09.2026, решение Влада: «кнопку
    #ручного перевода на курс нужно убрать, всё делается автоматически»).
    #
    #⚠️ Здесь, а не по таймеру и не по расписанию: учебный год меняется РАЗ В ГОД, и
    #единственный момент, когда это надо заметить, — первый запуск после 1 сентября.
    #Отдельный фоновый цикл на одноядерном сервере стоил бы дороже самой задачи.
    #
    #⚠️ Автомат смотрит на КАЛЕНДАРЬ и на собственные данные группы, а не на портал:
    #в день, когда портал лежит, он такой же верный (подробности — в course_rollover).
    #Сбой перевода не имеет права уронить запуск: журнал останется недоступен всему
    #колледжу из-за одной строки, а сам перевод идемпотентен и дождётся следующего старта.
    try:
        from . import course_rollover
        from .db import SessionLocal
        _db = SessionLocal()
        try:
            _rolled = course_rollover.autorun(_db)
            if _rolled:
                events.record("info", "course_rollover",
                              f"новый учебный год: переведено групп {len(_rolled)}")
        finally:
            _db.close()
    except Exception as e:                                          # noqa: BLE001
        events.record("warn", "course_rollover", f"автоперевод курса не выполнен: {e}")
