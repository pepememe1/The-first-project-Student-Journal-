...
#База у продукта ОДНА — SQLite: рядом с проектом при разработке, на бою тот же SQLite
#под SQLCipher (ключ ниже). Переменную оставляем, потому что ею пользуется локальный
#сервер десктопа: у каждого вошедшего своя копия базы (`db.rebind`), и путь к ней
#задаётся снаружи. Другой СУБД за этой строкой больше нет — поддержка PostgreSQL
#убрана 04.09.2026 целиком, см. шапку `db.py`.
...
#━━ ДЛИННАЯ СЕССИЯ ДЛЯ ТЕХ, У КОГО ВКЛЮЧЁН ВТОРОЙ ФАКТОР (03.09.2026) ━━━━━━━━━━━━━
#Требование Ярослава дословно: «сделай чтобы у тех кто подключил себе аутентификатор
#был токен месяц (на телефоне) и неделю на вебе / десктопе».
#
#Размен здесь честный и в нашу пользу. Пятичасовой потолок защищает ровно от одного:
#человек отошёл от ОБЩЕГО компьютера, не выйдя из аккаунта, а пароль — единственное,
#что стоит между чужим и журналом. Со вторым фактором пароля мало: подобравший или
#подсмотревший его не войдёт без кода с телефона. То есть длинная сессия покупается
#не ослаблением защиты, а её усилением в другом месте.
#
#⚠️ ПОТОЛОК ПЕРЕСЧИТЫВАЕТСЯ ОТ ТЕКУЩЕГО СОСТОЯНИЯ, а не от того, что было при входе.
#Отключил второй фактор — его сессии тут же судятся по короткой политике и умирают
#досрочно. Иначе «включу на минуту, получу месяц, выключу» было бы штатным обходом.
#
#⚠️ И обратное: это НЕ значит «месяц без единой проверки». Признак подозрительной
#активности (throttle.suspicion) обрывает тихое продление в любой момент. Длинный срок
#снимает рутину, а не контроль.
#⚠️ Здесь стояло «администратору второй фактор и так обязателен» — с 05.09.2026 это
#неправда: у admin фактор выключен совсем (`routers/mfa.py::is_active`), поэтому
#длинной сессии администратор не получает вовсе — живёт по короткой политике (22.09.2026).
JWT_MFA_TTL_MIN = int(os.environ.get("GRADEBOOK_JWT_MFA_TTL_MIN", "10080"))          # 7 суток
JWT_MFA_MOBILE_TTL_MIN = int(os.environ.get("GRADEBOOK_JWT_MFA_MOBILE_TTL_MIN", "43200"))  # 30 суток

#🔐 ДОВЕРЕННОЕ УСТРОЙСТВО (4.0, галочка «Доверять этому устройству?» на форме входа).
#Семантика подтверждена Ярославом: пока человек вошёл с доверенного устройства — сессия
#«без срока»; вышел — устройство ещё TRUSTED_DEVICE_IDLE_DAYS считается доверенным
#(вход без кода из письма); не вернулся за этот срок — доверие снимается.
#⚠️ «Без срока» у JWT физически невозможно — у токена обязан быть `exp`. Поэтому это год:
#потолок по возрасту у такой сессии снят (`/auth/refresh`), а год — страховка на
#случай, если человек выбросил телефон, не выйдя из аккаунта. Честная граница названа.
#⚠️ Короткий access при этом ПРЕЖНИЙ (пять часов): долгим становится только refresh,
#то есть украденный запрос с access-токеном живёт столько же, сколько раньше.
TRUSTED_SESSION_TTL_MIN = int(os.environ.get("GRADEBOOK_TRUSTED_SESSION_TTL_MIN", str(365 * 24 * 60)))
TRUSTED_DEVICE_IDLE_DAYS = int(os.environ.get("GRADEBOOK_TRUSTED_DEVICE_IDLE_DAYS", "15"))
#Сколько живёт код подтверждения входа из письма. Письмо идёт дольше, чем код из
#приложения, поэтому срок тот же, что у challenge второго фактора (10 минут).
LOGIN_CONFIRM_TTL_MIN = int(os.environ.get("GRADEBOOK_LOGIN_CONFIRM_TTL_MIN", "10"))
...
#🔑 БАРЬЕР ПОДТВЕРЖДЕНИЯ УСТРОЙСТВА: 'auto' (умолчание) или 'strict' (15.09.2026,
#требование Влада «нужно чтобы exe был подключен к бд автоматически»).
#
#Что было. Вход в программе шёл в два шага: пароль проверялся на бою как ВЕБ-клиент
#(барьер к нему не применяется) и проходил, а сразу за ним синхронизация шла туда же
#как ДЕСКТОП — и упиралась в 403. Человек с верным паролем видел «введите код, который
#выдаст администратор», причём администратор в этот момент — он сам.
#
#Что стало в режиме 'auto': устройство, с которого пришёл УЖЕ АУТЕНТИФИЦИРОВАННЫЙ
#запрос, заносится в одобренные само, один раз, с записью в журнал аудита.
#
#⚠️ ЦЕНА НАЗВАНА ПРЯМО: это ослабление §4.11. Барьер защищал от того, что укравший
#логин и пароль скачает журнал через `/sync/*` с чужой машины; теперь от этого
#защищают только сам пароль, анти-брутфорс и РОЛЕВОЙ СКОУП выдачи
#(`sync._scope_pull_for_role` — студенту приходят его же данные, а не вся база).
#Полного дампа колледжа по украденному студенческому паролю по-прежнему не будет.
#
#⚠️ Возврат к прежнему поведению — ОДНА переменная окружения, без правки кода и без
#новой сборки: `GRADEBOOK_DEVICE_APPROVAL=strict`. Настройкой, а не кодом, именно
#потому, что решение принимает заказчик, а живёт оно на его машине.
#🔥 ЗНАЧЕНИЕ СВЕРЯЕТСЯ СО СПИСКОМ, А НЕ С ОДНИМ «strict» (находка ревью T04, 18.09.2026).
#Было `DEVICE_APPROVAL_MODE != "strict"`, и у этого сравнения опасная асимметрия: любая
#опечатка — `strcit`, `Strict ` с лишним пробелом внутри, `srict` — означает «не strict»,
#то есть МОЛЧА включает автоодобрение. Администратор при этом уверен, что запер барьер:
#он его именно запирал, просто промахнулся по клавише. Ошибка в сторону ослабления
#защиты не имеет права быть тихой, поэтому неизвестное значение — отказ запуска с
#называнием того, что написано и что допустимо.
#⚠️ Отказ запуска — осознанный размен и он в нужную сторону: служба не поднимется и это
#увидят сразу, тогда как тихо ослабленный барьер не виден вообще никогда.
DEVICE_APPROVAL_MODES = ("auto", "strict")
DEVICE_APPROVAL_MODE = (os.environ.get("GRADEBOOK_DEVICE_APPROVAL", "auto").strip().lower()
                        or "auto")
if DEVICE_APPROVAL_MODE not in DEVICE_APPROVAL_MODES:
    raise RuntimeError(
        f"GRADEBOOK_DEVICE_APPROVAL={DEVICE_APPROVAL_MODE!r} — неизвестный режим. "
        f"Допустимо: {', '.join(DEVICE_APPROVAL_MODES)}. "
        "Оставьте пустым для режима auto (машина одобряется по верному паролю) "
        "или укажите strict (код подтверждения от администратора).")
DEVICE_AUTO_APPROVE = DEVICE_APPROVAL_MODE != "strict"
...
def issue_ttl_min(client: str, now=None, mfa: bool = False) -> int:
    ...
    android = (client or "").strip().lower() == "android"
    if mfa:
        #У второго фактора рубеж РОВНЫЙ, а не «до понедельника»: месяц и неделя —
        #то, что человеку обещано словами в настройках, и подгонять их под календарь
        #значило бы объяснять, почему обещанный месяц кончился в четверг.
        return JWT_MFA_MOBILE_TTL_MIN if android else JWT_MFA_TTL_MIN
    if not android:
        return JWT_TTL_MIN
    return max(JWT_MOBILE_FLOOR_MIN, minutes_until_next_monday(now))


def session_ttl_min(client: str, mfa: bool = False) -> int:
    ...
    if mfa:
        return JWT_MFA_MOBILE_TTL_MIN if android else JWT_MFA_TTL_MIN
    return JWT_MOBILE_TTL_MIN if android else JWT_TTL_MIN
