...
def _group_schedule(db: Session, group: str, category: str = "", cached_only: bool = False):
    ...
    #⚠️ Именованный аргумент передаём ТОЛЬКО когда он что-то меняет. Обычный путь
    #расписания обязан звать `get_group` ровно как раньше: полный прогон показал, что
    #безусловное `cached_only=` уронило четыре чужих теста публичного расписания — их
    #заглушки о новом параметре не знают. Это и есть цена молчаливой смены формы вызова
    #у функции, которую подменяют в стендах: сама правка была про сводку, а сломала
    #страницу, которая никуда не переезжала.
    kw = {"cached_only": True} if cached_only else {}
    data = schedule_web.get_group(group, category, **kw) if group else None
...
def today_digest(db: Session, user: User, today=None) -> tuple:
    ...
    #🔥 ТОЛЬКО КЭШ — СВОДКА НЕ ХОДИТ В СЕТЬ (21.09.2026). Она считается на пути
    #`GET /me/events`, который приложение опрашивает в фоне, а `get_group` при промахе
    #кэша идёт на ЧУЖОЙ портал с таймаутом 20 с. Кэш живёт 3 часа, значит после ночи он
    #протух у всех 106 групп разом — и каждый первый студент каждой группы занимал бы
    #поток на одноядерной машине ровно в утренний час пик.
    #⚠️ Урок не новый: ровно поэтому курс студента на главной читается через
    #`groups_by_course_cached`, а не напрямую. Здесь тот же горячий путь.
    #⚠️ Цена названа: пока расписание группы не попало в кэш (его наполняет любой заход
    #в расписание, виджет или админский импорт), сводка не уйдёт. Метку дня в этом
    #случае НЕ ставим — попробуем при следующем обращении, а не потеряем день целиком.
    #🔥 ХОЛОДНЫЙ КЭШ ОБЯЗАН ДАВАТЬ ЧЕСТНЫЙ НОЛЬ, А НЕ РАСПИСАНИЕ ИЗ ОДНИХ ПРАВОК
    #(нашёл Полковник 21.09.2026). `_apply_overrides` при `data is None` собирает
    #расписание ТОЛЬКО из админских правок — это верно для колледжа без портала, но
    #здесь означало бы вот что: кэш протух (каждое утро, TTL 3 ч), у группы есть замена
    #пары — и сводка говорит «Сегодня пар: 1» вместо настоящих пяти, выглядит настоящей,
    #а метка дня гасит правильную сводку до завтра. Спрашиваем СНИМОК напрямую, и пустой
    #кэш отличаем от пустого расписания.
    snapshot = schedule_web.get_group(group, cat, cached_only=True)
    if snapshot is None:
        return 0, ""          #расписания ещё нет в кэше — сегодня промолчим, но день не сожжём
    data = _group_schedule(db, group, cat, cached_only=True)
    ...
    #
    #🔥 ДЕНЬ БЕРЁМ ПО ЛОКАЛЬНОМУ ВРЕМЕНИ СЕРВЕРА, А НЕ ПО UTC (починено 21.09.2026).
    #Здесь стояло `datetime.now(timezone.utc)`, и это расходилось с ЧЁТНОСТЬЮ НЕДЕЛИ:
    #её считает `schedule_web.current_week_parity()` от `date.today()`, то есть по
    #локальному календарю машины. Боевой сервер живёт в Europe/Moscow (проверено
    #`date` на бою), поэтому каждые сутки с 00:00 до 03:00 МСК день недели брался
    #ВЧЕРАШНИЙ, а неделя — сегодняшняя: человек получал сводку за другой день, и
    #выглядела она совершенно настоящей. В Улан-Удэ это окно приходится на 05:00–08:00
    #утра — ровно на то время, когда «что у меня сегодня» и спрашивают.
    #⚠️ Правило шире этой функции: день и чётность недели обязаны браться из ОДНОГО
    #календаря. Две копии «сегодня» расходятся молча, и расходятся именно ночью.
    _when = today or datetime.now()
    idx = _when.weekday()                       #0 — понедельник, локально как у расписания
    if idx >= len(WEEKDAYS):
        return 0, ""
    day_key = WEEKDAYS[idx]
    #⚠️ ЧЁТНОСТЬ СЧИТАЕМ ОТ ТОГО ЖЕ ДНЯ, что и `idx` (нашёл Полковник). Здесь стоял
    #`current_week_parity()` без аргумента — то есть всегда от `date.today()`, и вызов с
    #явной датой давал день недели из одного календаря, а неделю из другого: смесь двух
    #недель, выглядящая как настоящее расписание. Продуктового вызова с `today` пока нет,
    #но первый же (сводка «на завтра», пересчёт задним числом) получил бы чужие пары.
    week = schedule_web.current_week_parity(_when.date() if hasattr(_when, "date") else _when)
