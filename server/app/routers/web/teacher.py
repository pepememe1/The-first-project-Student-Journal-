...
def teacher_journal(group: str = Query(...), subject: str = Query(...),
    ...
    return {
        ...
        "lessons": [{"id": l.id, "type": l.type, "number": l.number,
                    ...
                    for l in lessons],
        "students": rows,
        #Параметры методики, по которым сложен средний балл. Нужны МОБИЛЬНОМУ клиенту:
        #без сети он пересчитывает средний сам (тем же портом grading.py, закреплённым
        #общим контрактом), иначе преподаватель выставляет оценки, а колонка не шевелится.
        #Без этих трёх чисел пересчёт шёл бы по умолчаниям и разошёлся бы с сервером у
        #любого колледжа, который методику правил. Не ПДн: это правила подсчёта, они и так
        #показываются студенту текстом в его журнале.
        "methodology_cfg": {k: W.grading.avg_config(cfg)[k]
                            for k in ("avg_absence_weight", "avg_count_absence",
                                      "avg_include_exam")},
