...
def mod_conversations(q: str = Query(""), kind: str = Query(""),
    ...
    """
    #🔒 БЕЗ `kind` СПИСОК ОТДАВАЛ ВСЕ БЕСЕДЫ КОЛЛЕДЖА (вторая половина находки M01):
    #участников поимённо и текст последнего сообщения каждой личной переписки. Для
    #очереди обращений это не нужно вовсе — она смотрит ровно в служебные беседы, — а
    #как способ полистать чужие разговоры годилось вполне. Умолчание сужено до
    #`moderation`; произвольный тип по-прежнему запрашивается явно, но только из
    #перечисленных, чтобы «фильтр» не превращался обратно в «всё подряд».
    _ALLOWED_KINDS = ("moderation", "report")
...
def mod_conversation_messages(conv_id: str, report_id: int = Query(0), request: Request = None,
    ...
    #🔥 ОСНОВАНИЕ ДОСТУПА ОБЯЗАТЕЛЬНО, И ЕГО НЕЛЬЗЯ УБРАТЬ ИЗ ЗАПРОСА (находка M01).
    #Раньше `report_id` был НЕОБЯЗАТЕЛЬНЫМ, и `_require_live_ticket` звался только когда
    #он пришёл. То есть замок открывался тем, что его не трогают: модератор убирал
    #параметр из адреса — и читал любую личную переписку колледжа целиком, вместе с
    #удалёнными сообщениями и всеми прежними редакциями. Правило «по закрытой жалобе
    #переписку не смотрят» при этом выглядело действующим и было записано в документах.
    #
    #Оснований ровно два, и второе не лазейка, а сам смысл беседы:
    #  • ЖИВОЙ тикет, привязанный ИМЕННО к этой беседе, — расследование жалобы;
    #  • беседа С САМОЙ МОДЕРАЦИЕЙ (`kind == "moderation"`) — человек писал сюда, чтобы
    #    его прочитали, и очередь обращений именно этим и занимается.
    #Всё остальное — чужая переписка, и открывать её «просто так» нельзя даже с ролью.
    if report_id:
