...
def _notify_today_lessons(db: Session, user: User) -> int:
    ...
    ⚠️ ЗДЕСЬ, а не по расписанию, и по той же причине, что напоминания: планировщик ради
    одной сводки означал бы вечный фоновый поток на одноядерной машине. Проверка стоит
    там, где человек (или его приложение, опрашивающее уведомления) и так пришёл.

    ⚠️ ЧЕСТНАЯ ГРАНИЦА, и её надо называть вслух: сводка отправляется ПРИ ОБРАЩЕНИИ. Кто
    сегодня не открывал приложение и чьё устройство не опрашивало события, тот её и не
    получит. Настоящая утренняя рассылка «всем к 8:00» требует планировщика — это
    отдельная работа, и делать вид, что она уже есть, нельзя.

    ⚠️ Метка «уже отправляли сегодня» живёт в общем состоянии со сроком до конца суток:
    при двух процессах словарь в памяти дал бы по одной сводке на каждый.

    Полностью в try/except: сбой сводки не имеет права лишить человека уведомлений об
    оценках, ради которых он сюда и пришёл.
    """
    try:
        if (user.role or "") != "student":
            return 0          #у преподавателя пары в нескольких группах — это другая сводка
        from .. import shared_state
        #Считает тот же код, что строит расписание на экране (портал + админские
        ...
        print(f"[events] сводка «пары на сегодня» не отправлена: {e}")
        return 0
...
def list_events(filter_: str = Query("unread", alias="filter"),
    ...
    _notify_today_lessons(db, user)      #сводка «что у меня сегодня», раз в сутки
