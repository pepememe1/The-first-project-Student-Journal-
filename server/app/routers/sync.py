...
#Поля, которые НЕЛЬЗЯ обнулить входящей пустотой (см. пояснение в месте применения).
...
def push(payload: dict = Body(...), request: Request = None,
    ...
    for name, items in changes.items():
        ...
        for item in items:
            ...
            #Правило общее: клиент может ЗАДАТЬ секрет, но не может ОБНУЛИТЬ то, чего не
            #знает. Смена пароля идёт своими эндпоинтами, а не попутно синхронизацией.
            item = {k: v for k, v in item.items()
                    if not (k in _NEVER_BLANK and not v and getattr(existing, k, ""))}
