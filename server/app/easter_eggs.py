...
def pick_avatar_egg(user, db: Session) -> str | None:
    ...
    🔥 ВЫБОР ДЕТЕРМИНИРОВАННЫЙ — по паре (человек, день), а НЕ бросок (24.08.2026).
    ...
    ⚠️ Ключ выбора — id и ДАТА, а не только id. Постоянная на всю жизнь метка означала
    бы, что половина колледжа не увидит вторую отсылку никогда; смена по дню оставляет
    выбор стабильным ровно там, где стабильность и нужна — в пределах одного дня.
    """
    if user.role != "student":
        return None
    #hashlib, а не hash(): встроенный солится случайно при каждом старте процесса, и
    #метка менялась бы после каждого перезапуска сервера.
    seed = f"{user.id}|{_local_now().date().isoformat()}".encode()
    egg = AVATAR_EGGS[hashlib.sha256(seed).digest()[0] % len(AVATAR_EGGS)]
