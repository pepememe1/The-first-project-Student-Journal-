...
Что внутри отчёта по КАЖДОЙ группе:
  ...
  • посещаемость: Н (неявки), Б (болезнь), О (ОПОЗДАНИЯ) и всего пропусков.
    ⚠️ «Проп.» = Н + Б: опоздание пропуском НЕ считается (3.7.6) — студент на
    занятии был. Столбец опозданий остаётся отдельно: систематические опоздания —
    сигнал куратору, терять его нельзя;
...
def collect_group(db, group: str, year: str, semester: int, cfg: dict,
    ...
    subjects = {l.subject for l in lessons if l.subject}
    #⚠️ Живой баг («в отчёте видны только предметы, по которым есть оценки») — до сих
    #пор список ВСЁ РАВНО собирался из занятий, а предмет БЕЗ единой пары/оценки (план
    #есть, преподаватель ещё не открыл журнал) в него не попадал — куратору был нужен
    #противоположный сигнал: «по этому предмету пока пусто», а не тишина. Досыпаем
    #предметы ДЕЙСТВУЮЩЕГО плана — их пустая строка (avg=0) в отчёте и есть ответ
    #«ничего не выставлено». АРХИВ не трогаем — план мог с тех пор замениться.
    if not is_archive:
        subjects |= W.group_plan_subjects(db, group, cfg)
...
#Пороги 4.5/3.5/2.5 — ТЕ ЖЕ, что приложение использует для цвета и настроения маскота по
#среднему баллу. Свой набор порогов здесь НЕ придумываем — иначе одна и та же оценка
#красилась бы по-разному на разных экранах (§12 отчёта куратора в мессенджере).
#⚠️ Список мест, который тут стоял (vector/mascot.py, ui/dashboards.py,
#ui/teacher_dashboard.py), устарел целиком: все три модуля удалены вместе с Qt. Живые
#соседи сегодня — `web/src/components/vector/GradesOverview.vue` и
#`CuratorReportOverlay.vue`; правишь пороги здесь — проверь их.
...
def build_xlsx(groups_data: list, term: dict) -> bytes:
    ...
    for gd in groups_data:
        ...
        headers = ["№", "Студент"] + subs + ["Общий", "Н", "Б", "Оп.", "Проп.", "Долги"]
...
def build_docx(groups_data: list, term: dict) -> bytes:
    ...
    for gi, gd in enumerate(groups_data):
        ...
        subs = gd["subjects"]
        headers = ["№", "Студент"] + subs + ["Общий", "Н", "Б", "Оп.", "Долги"]
