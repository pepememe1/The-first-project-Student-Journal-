...
class LoginIn(BaseModel):
    login: str
    password: str
    #4.0: секрет доверенного устройства (выдан при прошлом входе, лежит у клиента) и
    #галочка «Доверять этому устройству?». Оба НЕОБЯЗАТЕЛЬНЫ: десктоп и старые клиенты
    #их не шлют и обязаны входить как раньше.
    trust_token: str = ""
...
class TokenOut(BaseModel):
    ...
    refresh_token: str = ""
    #4.0: секрет доверенного устройства — только когда его ВЫДАЛИ на этом входе (галочка
    #«Доверять этому устройству?»). Клиент кладёт его к себе и предъявляет при следующем
    #входе. Пусто — ничего нового не выдано.
    trust_token: str = ""
