...
#Где Argos держит установленные модели. Каталог читаем САМИ, не спрашивая библиотеку, —
#почему именно так, объяснено в докстринге `status()`. Переменную окружения уважаем: на
#машине ВСГУТУ модели, скорее всего, лягут не в домашний каталог службы.
def _package_dirs() -> list:
    """Каталоги, где могут лежать модели. Список, а не один путь, и это не перестраховка.

    ⚠️ Argos держит пакеты в `~/.local/share/argos-translate/packages` НА ВСЕХ системах,
    включая Windows, — проверено у живой библиотеки (`settings.package_data_dir`). Первый
    вариант этой функции брал на Windows `%LOCALAPPDATA%` «как принято у остальных
    программ» и получал ложное «моделей нет» на машине, где стоят все четыре пары.
    Ошибка ровно того сорта, который здесь опаснее всего: ответ выглядит осмысленным,
    кнопка перевода честно прячется — и никто не догадывается, что переводчик исправен.
    Поэтому смотрим ОБА места, а не угадываем одно."""
    override = os.environ.get("ARGOS_PACKAGES_DIR", "").strip()
    if override:
        return [override]
    out = [os.path.expanduser("~/.local/share/argos-translate/packages")]
    #Второй кандидат — на случай сборки, которая всё же уважает системную раскладку.
    base = os.environ.get("LOCALAPPDATA") if sys.platform == "win32" else None
    if base:
        out.append(os.path.join(base, "argos-translate", "packages"))
    return out


def _pairs_on_disk() -> list:
    """Пары языков по метаданным установленных пакетов — БЕЗ импорта argostranslate.

    Каждый пакет это каталог с `metadata.json`, где записаны `from_code`/`to_code`.
    Читаем их сами: см. `status()` — импорт пакета стоит четверть гигабайта, и платить
    её за ответ на вопрос «а переводчик вообще есть» нельзя."""
    out = []
        ...
        for name in names:
            ...
            try:
                with open(meta, encoding="utf-8") as fh:
                    box = json.load(fh)
                src, dst = box.get("from_code"), box.get("to_code")
            except Exception:
                continue
            #Показываем только те пары, которыми продукт реально умеет пользоваться:
            #установленный вручную суахили ничего не значит, пока его нет в LANGUAGES.
            if src in LANGUAGES and dst in LANGUAGES and src != dst:
                out.append(f"{src}->{dst}")
    return sorted(set(out))


def status() -> dict:
    """Что умеет ЭТА машина: {available, installed, models, pairs, source, reason}.

    🔑 ПЕРЕВОД — СПОСОБНОСТЬ ХОСТА, ровно как распознавание речи (§5.2.1) и озвучка
    (§5.3). Клиент не решает, чем переводить, и не держит своей копии этого знания: он
    спрашивает тот сервер, с которого открыт, — а внутри программы это локальный сервер
    на 127.0.0.1. Поэтому одна и та же сборка честно ведёт себя по-разному на боевом
    VPS, на машине ВСГУТУ и в десктопе, без единой настройки.

    🔥 ЗАЧЕМ ЭТА ФУНКЦИЯ ВООБЩЕ ПОЯВИЛАСЬ (02.09.2026). Кнопка «перевести» показывалась
    ВСЕГДА и на бою не работала НИКОГДА: пакета там нет и быть не может (замер ниже).
    То есть продукт предлагал действие, которое гарантированно заканчивалось отказом, —
    а человек, нажав и получив ошибку, делает вывод «сломалось», а не «здесь этого нет».
    Разница между «не умеет» и «сломалось» — не косметика: во втором случае жмут ещё раз
    и идут жаловаться.

    ⚠️ ОТВЕТ ОБЯЗАН БЫТЬ ДЕШЁВЫМ, И ЭТО НЕ ПРИДИРКА. `import argostranslate` стоит
    **252.7 МБ** ещё до единой модели (замер `tools/measure_argos_memory.py`, 02.09.2026)
    — библиотека тянет за собой stanza. Спроси мы у неё «ты установлена?», сам вопрос
    занимал бы четверть гигабайта у КАЖДОГО, кто просто открыл мессенджер, даже если
    переводить он не собирался. Поэтому наличие пакета проверяется `find_spec` (модуль
    при этом НЕ импортируется, проверено), а список пар читается с диска.

    ⚠️ ИСТОЧНИКОВ ОТВЕТА ДВА, И РАСХОЖДЕНИЕ МЕЖДУ НИМИ НАЗВАНО, А НЕ ПОДРАЗУМЕВАЕТСЯ.
    Если библиотека УЖЕ импортирована (кто-то переводил в этом процессе), спрашиваем её
    — она авторитетна и стоит уже ноль. Если нет — читаем метаданные пакетов с диска, и
    это может разойтись с правдой: битую модель метаданные не отличат от целой. Что
    именно ответило, видно в поле `source`; молча выдавать одно за другое нельзя.
    """
    installed = importlib.util.find_spec("argostranslate") is not None
    if "argostranslate.translate" in sys.modules:
        pairs, source = installed_pairs(), "library"
    else:
        pairs, source = _pairs_on_disk(), "disk"

    if not installed:
        #Причина НАЗЫВАЕТ действие. «Недоступен» отправляет админа проверять сеть,
        #которой в этом модуле нет вовсе (§ про удаление Google).
        reason = ("Переводчик не установлен на этом сервере. "
                  "Пакет argostranslate и модели ставятся отдельно "
                  "(tools/install_argos_models.py).")
    elif not pairs:
        reason = ("Переводчик установлен, но модели языков не скачаны "
                  "(tools/install_argos_models.py).")
    else:
        reason = ""
    return {"available": bool(installed and pairs), "installed": installed,
            "models": bool(pairs), "pairs": pairs, "source": source, "reason": reason}
...
def translate(text: str, dst: str, src: str = AUTO) -> dict:
    ...
    #Честный отказ с ПРИЧИНОЙ, а не «временно недоступен»: пакета нет — значит его надо
    #поставить, и администратор должен прочитать именно это, а не гадать про сеть,
    #которой здесь больше нет вовсе.
    #⚠️ Текст причины берём У `status()`, а не пишем второй раз здесь. Две копии одного
    #объяснения разъезжаются молча, и хуже всего то, КАК именно: кнопка в интерфейсе
    #(она спрашивает status) говорила бы одно, а ответ на нажатие — другое.
    #⚠️ Спрашиваем ДО импорта: у машины с пакетом, но без моделей, прежняя проверка
    #сначала платила 252.7 МБ за импорт и только потом отказывала.
    state = status()
    if not state["available"]:
        return {"ok": False, "text": "", "detected": detected, "reason": state["reason"]}
