...
#🔥 Почему причина отказа хранится ОТДЕЛЬНО, а не собирается вызывающим.
#28.08.2026 собранный .exe не открывал окно и показывал человеку: «чаще всего не хватает
#WebView2 Runtime — установите его». Runtime стоял. Настоящая причина (в сборку не попал
#корневой модуль `vector_nlu`, локальный сервер падал на импорте) была видна ТОЛЬКО в
#gradebook.log, до которого обычный пользователь не дойдёт. `run()` возвращала голый
#False, и main.py дописывал «движок недоступен ИЛИ сервер не поднялся» — то есть честно
#не знал, что случилось, и называл наугад более частый вариант. Диагноз наугад хуже
#отсутствия диагноза: он отправляет человека чинить исправное.
_FAIL_REASON = ""


def fail_reason(text: str = "") -> str:
    """Записать/прочитать причину, по которой окно открыть не удалось."""
    global _FAIL_REASON
    if text:
        _FAIL_REASON = text
    return _FAIL_REASON
...
def run() -> bool:
    ...
    if not available():
        ...
        fail_reason("не установлен системный движок Microsoft Edge WebView2 Runtime")
        return False
    ...
    if not api.start():
        ...
        fail_reason(f"локальный сервер не поднялся: {api.error}")
        return False
